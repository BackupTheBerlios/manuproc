...
std::cout << "\tFehler";
/*
   if(analyse_only) analyse("Von Eltern benˆtigte Menge (EM="+ElternMenge.String()+") stimmt nicht mit \n"
...
//std::cout << Msum<<'\t'<<AE.getStueck()<<'\t'<<AE.getRestStk()<<'\n';
     if(analyse_only) { 
            analyse("Zuord.-Summen ist grˆﬂer als Auftragssumme",AE,Msum,AE.getStueck());
            return false; }
     else assert(!"nicht implementiert\n");  
   }
  return true;
...
bool ppsInstanzReparatur::check_F_dispo(const int uid,const bool analyse_only,const AufEintrag &AE,const ABmt &Msum) const
{
  if(Msum>AE.getRestStk())
   { 
     if(analyse_only) { std::cout << "Analyse: Zuord.-Summen ("<<Msum<<") sind grˆﬂer als ("<<AE.getRestStk()<<") f¸r "<<AE<<'\n';
                        return false; }
     else assert(!"nicht implementiert\n");  
   ...
   for(SelectedFullAufList::iterator i=AL1.begin();i!=AL1.end();++i)
    {
      ...
      if(!L.empty())
       {
         if(analyse_only) {analyse("Analyse: Kundenauftrag und 2er d¸rfen keine Eltern haben.\n",*i);
                        return false; }
