...
#ifdef HAS_ADDR_GROUP_Kunden	
			KundengruppeID::Kunden
#else
			KundengruppeID::None
#endif
			) throw(SQLerror);
...
//        bool isAuftragsadresse() const;
       ...
       bool isInGrp(const std::string groupname) const throw(SQLerror); 
        ...
        Kundengruppe::ID ownsAGrp(const std::string obergruppe) const throw(SQLerror);
  ...
  fixedpoint<2> getProvSatz_Artikel(const ArtikelBase art,
...
private:
	...
	mutable std::string iban;  // nr fr LCR Verfahren (z.B. Frankreich)
...
public:
 ...
 std::string fixeNotiz(const NotizZiel nz) const throw(SQLerror);
