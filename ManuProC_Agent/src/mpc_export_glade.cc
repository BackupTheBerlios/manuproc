...
{  
   ...
   export_close = Gtk::manage(new class Gtk::Button());
   ...
   export_close->set_flags(Gtk::CAN_FOCUS);
   export_close->set_flags(Gtk::CAN_DEFAULT);
   export_close->set_relief(Gtk::RELIEF_NORMAL);
   export_close->add(*alignment5);
   ...
   mpc_export->add_action_widget(*export_close, -7);
   image5->show();
   label23->show();
   hbox15->show();
   alignment5->show();
   ...
   vbox4->show();
   mpc_export->show();
   file_select->signal_clicked().connect(SigC::slot(*this, &mpc_export_glade::on_file_select_clicked), false);
   do_file_export->signal_clicked().connect(SigC::slot(*this, &mpc_export_glade::on_do_file_export_clicked), false);
   do_file_send->signal_clicked().connect(SigC::slot(*this, &mpc_export_glade::on_do_file_send_clicked), false);
}
