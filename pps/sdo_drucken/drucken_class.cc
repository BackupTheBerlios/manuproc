...
#ifdef MABELLA_EXTENSIONS
	bool bez_in_2nd=false;
        if(ArtikelTyp::hasAttribute(schema_mem->Typ(),
     		ArtikelTypAttr::mit_bezeichnung) &&
           ean_code && !Configuration.order_clausel.empty()
          )
          { 
           if(zeilen_passen_noch%2) os << "\\shaderow";
           os << "&&";
           ...
           if(rabatt_bool &&  !Configuration.order_clausel.empty()) 
	     bs=bezstring.substr(0,15);
	   else
	     bs=bezstring.substr(0,30);
           os << "\\multicolumn{4}{l}{\\scriptsize "<<Gtk2TeX::string2TeX(bs)<<"}";
           bez_in_2nd=true;
          }

	if(schema_own->Id() != schema_mem->Id() && 
	   (ean_code || !Configuration.order_clausel.empty()) && Typ()!=Lieferschein)
	  {
	   cH_ArtikelBezeichnung own_bez(artikelbase,schema_own->Id());
	   if(bez_in_2nd)
	     os << "&";
   ...
   {  neue_spalte( erste_spalte, os);
	...
	if(s->Id()==ExtBezSchema::default_id)
		...
		if(rabatt_bool &&  !Configuration.order_clausel.empty())
		  ...
		  bs=bezstring.substr(0,30);
	        if(!ean_code || Configuration.order_clausel.empty())
	          os << "\\scriptsize " << Gtk2TeX::string2TeX(bs);
		else
		  os << " ";
	      }
	   }
  ...
  if(schema_mem->Id() != schema_own->Id())
    {
     for(ExtBezSchema::const_sigiterator j=schema_own->sigbegin(1);
      ...
      { tabcolumn += j->TeXtabformat ; ++spaltenzahl ; 
        if((!ean_code && Configuration.order_clausel.empty()) || Typ()==Lieferschein)
          ueberschriften += "&\\mbox{"+ug + j->bezkomptext+"}";
	else
          ueberschriften += "&";
      }    
...
// ggf. Zweite Kopfzeile erzeugen
  ...
  if(ean_code && !Configuration.order_clausel.empty() && Typ()!=Lieferschein)
   {
    os << "\n\\\\[-1ex]&\\multicolumn{1}{l}{\\scriptsize{}}";
    os << "&\\multicolumn{5}{l}{\\scriptsize{";
    for(ExtBezSchema::const_sigiterator j=schema_mem->sigbegin(BEZEICHNUNG_SIGNIFIKANZ);
  			j!=schema_mem->sigend(BEZEICHNUNG_SIGNIFIKANZ);++j)
      os << " " << mld->MLT(MultiL_Dict::TXT_BEZEICHNUNG,j->bezkomptext) 
		<< " " << j->separator;
    os << "}}";
    bez_in_2nd=true;
   }
  ...
  if(schema_mem->Id() != schema_own->Id() && 
    ...
    {
     if(!bez_in_2nd)
       {os << "\n\\\\[-1ex]&\\multicolumn{3}{l}{\\scriptsize{}}";
        os << "&\\multicolumn{4}{l}{\\scriptsize{";
       }
     else os << "&\\multicolumn{3}{l}{\\scriptsize{";
     for(ExtBezSchema::const_sigiterator j=schema_own->sigbegin(1);
     				j!=schema_own->sigend(1);++j)
 ...
 else
  {
   ...
   if (page_counter==1) // 1. Seite
	...
	if(Typ()==Extern)
		...
		os << kunde_von->land()->LKZ() << " - "
		   << string2TeX(kunde_von->plz()+" "+kunde_von->ort())+"\\\\\n\n";
		os << "\\smallskip\n";   		
		}	
...
//   if(!Rueckstand())
   ...
   if (page_counter==1 && (Typ()==Lieferschein || Typ()==Rechnung))
    {os <<"\\scriptsize{"<<string2TeX(mld->MLT(MultiL_Dict::TXT_SIEERHIELTEN))<<"}";
     if(Typ()==Rechnung)
