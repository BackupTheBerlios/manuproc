...
{
   // dispo (2er) Auftrag anlegen bei �erplanung
   ...
   if (menge>getRestStk())
    { assert(!Instanz()->LagerInstanz());
      ...
      // nur soviel unten verwenden (tats�hlich zu uns ziehen,
...
// existiert, Menge erh�en. So kann man von Auftr�e aus verschiedenen
...
void AufEintrag::Ueberplanen(const ArtikelBase& artikel,mengen_t menge,const ManuProC::Datum &datum)
{
 ManuProC::Trace _t(trace_channel, __FUNCTION__,str(),
   ...
   // zus�zliche Menge vermerken und Material bestellen
...
// quelle und seine Kinder wurden bereits ver�dert
...
{ ManuProC::Trace _t(trace_channel, __FUNCTION__,NV("this",*this),NV("M",M));
  ...
  {  if (M>=0) ArtikelInternNachbestellen(M);
    ...
    {  assert(getCombinedStatus()==OPEN);
       ...
       Transaction tr;
       mengen_t m=M;
       // Menge wird gleich h�pchenweise wieder hinzugebucht
...
{ManuProC::Trace _t(trace_channel, __FUNCTION__,NV("Datum",ld));
 Transaction tr;
 Query("lock auftragentry in exclusive mode"); // unn�ig? CP
