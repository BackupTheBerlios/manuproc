...
{ ManuProC::Trace _t(trace_channel, __FUNCTION__,NV("this",*this),
  ...
  if(new_status==OPEN || ((status==OPEN || status==CLOSED) && new_status==STORNO))
   {
    ...
    {  assert(NurEinKind(VZusatz));
    ...
    else 
     {
       ...
       if(abmenge>0) 
       {if (status==UNCOMMITED) // erstmaliges Anpassen an AuftrÃ¤ge
        ...
        // was ist mit 0ern im Einkauf, diese werden auch bei Ãœberproduktion nicht
  ...
  //	damit der Erste eventuelle LagerbestÃ¤nde reservieren kann!
